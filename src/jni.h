 ...
 */
typedef struct jni_native_method {
    const char *name;
...
struct JNINativeInterface_ {
    ...
    jobject (JNICALL *NewDirectByteBuffer)(JNIEnv *env, void *address0, jlong capacity);
...
struct JNIEnv_ {
    ...
    jobject NewDirectByteBuffer(void* address0, jlong capacity) {
        return functions->NewDirectByteBuffer(this, address0, capacity);
    }
...
JNIEXPORT jint JNICALL JNI_CreateJavaVM(JavaVM **pvm, JNIEnv **penv, JavaVMInitArgs *args);
...
#define JNI_THROW(_env, _excep_class_name, msg) \
    (_env)->ThrowNew((jclass) (load_boot_class(_excep_class_name)->java_mirror), msg)

#define JNI_THROW_NPE(_env, msg) \
    JNI_THROW(_env, "java/lang/NullPointerException", msg)
...
#define JNI_THROW_ArrayIndexOutOfBoundsException(_env, msg) \
    JNI_THROW(_env, "java/lang/ArrayIndexOutOfBoundsException", msg)
